...
def login():
    ...
    if not bcrypt.checkpw(password.encode(), user["password_hash"].encode()):
        query(
            """INSERT INTO events (event_type, user_id, ip_address, success, asset_id, session_id)
    ...
    session_rows = query(
        """INSERT INTO sessions (user_id, org_id, session_token, ip_address, device_info, login_time, sta_tus)
           VALUES (%s, %s, %s, %s, %s, NOW(), 'Active')
           RETURNING session_id""",
        (user["user_id"], user["org_id"], token, ip, device)
    )
    session_id = session_rows[0]["session_id"] if session_rows else 1

    query(
        """INSERT INTO events (event_type, user_id, session_id, ip_address, success, asset_id)
...
def logout():
    ...
    if token:
        query(
            "UPDATE sessions SET logout_time=NOW(), sta_tus='Expired' WHERE session_token = %s",
            (token,)
        )
